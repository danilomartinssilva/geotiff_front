...
import { doUpload } from "./save-file-actions";

export const addRaster = (input) => {
  return (dispatch) => {
    dispatch(startLoading("Loading Raster"));
    return RasterService.createRaster(input).then(
      (raster) => {
        dispatch(stopLoading());
        dispatch(doUpload(input));
      ...
      (error) => {
        ...
        dispatch(
          showAlert(
            `${SITE_CONFIG.title} was unable to load the tiff. Please make sure your url or file is accurate and try again.`
          )
        );
      }
    );
